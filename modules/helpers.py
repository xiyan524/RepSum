...
def index_mask(mask_row, mask_col, index):
    A = torch.zeros((mask_row, mask_col))
    B = index.float()
    AA = 1 - A
    seq_len = torch.sum(AA, dim=-1)
    word_offset = torch.cumsum(seq_len, dim=0).cuda()
    BB = B + word_offset.unsqueeze(dim=-1)

    flag_A = A.view(-1)
    flag_BB = BB.view(-1).long()
    flag_BB = torch.sub(flag_BB, mask_col)
    flag_A[flag_BB] = 1
    A = flag_A.view(mask_row, -1)
