...
class SeqReader(nn.Module, RecurrentHelper):
    def __init__(self, ntokens, **kwargs):
        ...
        # Layers
        ############################################
        self.embed = Embed(ntokens, self.emb_size, noise=self.embed_noise, dropout=self.embed_dropout)
...
class AttSeqDecoder(nn.Module):
    ...
    def get_embedding(self, step, trg, logits, sampling_prob, argmax, hard, tau):
        """
