...
class BaseTrainer:
    def __init__(self, train_loader, valid_loader,
        ...
        if batch_end_callbacks is None:
            ...
            self.batch_end_callbacks = [c for c in batch_end_callbacks if callable(c)]
    ...
    def _tensors_to_device(self, batch):
        """batch_trans = []
        for sample in batch:
            batch_trans.append([i for item in sample for i in item])
        res_tmp = list(map(lambda x: x.to(self.device), batch_trans))"""
