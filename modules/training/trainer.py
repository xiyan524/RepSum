...
class Trainer(BaseTrainer):
    ...
    def train_epoch(self, pre_train_epoch, batch_num, writer):
        """
        ...
        * This will be a pessimistic approximation of the true loss
        ...
        for i_batch, batch in enumerate(iterator, 1):
            ...
            # pre-train enc1-dec3 by self-supervised training
            ...
            writer.add_scalar('Train/loss', loss_sum, self.step)
            loss_count = 0
            writer.add_scalar('Train/nsent1_loss', loss_list[loss_count], self.step)
            if self.config["model"]["n_sent_sum_loss"]:
                loss_count += 1
                writer.add_scalar('Train/nsent2_loss', loss_list[loss_count], self.step)
            if self.config["model"]["prior_loss"]:
                loss_count += 1
                writer.add_scalar('Train/lm_loss', loss_list[loss_count], self.step)
            if self.config["model"]["topic_loss"]:
                loss_count += 1
                writer.add_scalar('Train/topic_loss', loss_list[loss_count], self.step)
            if self.config["model"]["length_loss"]:
                loss_count += 1
                writer.add_scalar('Train/length_loss', loss_list[loss_count], self.step)
            if self.config["model"]["doc_sum_kl_loss"]:
                loss_count += 1
                writer.add_scalar('Train/kl_loss', loss_list[loss_count], self.step)
            if self.config["model"]["doc_sum_sim_loss"]:
                loss_count += 1
                writer.add_scalar('Train/doc_sim_loss', loss_list[loss_count], self.step)
            if self.config["model"]["sum_loss"]:
                loss_count += 1
                writer.add_scalar('Train/sum_loss', loss_list[loss_count], self.step)
            if self.config["model"]["nsent_classification"]:
                loss_count += 1
                writer.add_scalar('Train/cls_loss', loss_list[loss_count], self.step)
            if self.config["model"]["nsent_classification_sum"]:
                loss_count += 1
                writer.add_scalar('Train/cls_sum_loss', loss_list[loss_count], self.step)
            if self.config["model"]["nsent_classification_kl"]:
                loss_count += 1
                writer.add_scalar('Train/cla_kl_loss', loss_list[loss_count], self.step)
            writer.flush()
            ...
            for c in self.batch_end_callbacks:
                if callable(c):
                    c(batch, losses, loss_list, batch_outputs, self.epoch)
        try:
