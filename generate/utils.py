...
def devectorize_generate(data, id2tok, eos, strip_eos=True, oov_map=None, pp=True):
    if strip_eos:
        for i in range(len(data)):
            try:
                data[i] = data[i][:list(data[i]).index(eos)]
    ...
    for sample in data:
        ...
        for sent in sample:
            sent = sent.tolist()
            for wrd in sent:
                wrd_real = id2tok.get(wrd, "<unk>")
                if wrd_real != "<PAD>":
                    sample_rst.append(wrd_real)
        results.append(sample_rst)
    data = results

    if oov_map is not None:
        #data = [[m.get(x, x) for x in seq] for seq, m in zip(data, oov_map)]
        data_tmp = []
        for seq, m in zip(data, oov_map):
            tmp = []
            for x in seq:
                tmp.append(m.get(x, x))
            data_tmp.append(tmp)
        data = data_tmp
